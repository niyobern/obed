...
export default function AnswerCard({ question, slug, number }){
        ...
        <div className="bg-white rounded w-full h-fit flex flex-col">
            <div className="flex flex-col gap-2 p-2 md:pt-4">
                <div className="border border-gray-200 rounded relative">
                    <span className="absolute p-1 rounded text-xs font-medium">IKIBAZO {number}</span>
                ...
                <div className="flex flex-col">
                   {question.choice !== question.answer && (
                    <div className="bg-red-200 cursor-pointer flex items-center gap-4 border-x border-gray-200 p-2 py-3 border-b-2 border-b-blue-600">
                        ...
                        <span>{question.answers[question.choice - 1]}</span>
                    </div>
                   )}
                    ...
                    <div key={index} className="cursor-pointer flex items-center gap-4 border-x border-gray-200 p-2 py-3 border-b-2 border-b-blue-600">
                        {index + 1 === question.answer && <AiFillCheckCircle color="green" size={30}/>}
                        {index + 1 !== question.answer && <AiFillCheckCircle color="white" size={30}/>}
                        <span>{item}</span>
                    </div>
                    )}
                    {(question.answers[0].slice(-3) === "png" || question.answers[0].slice(-3) === "gpg") && question.answers.map((item, index) => 
                    <div key={index} className={`${question.choice === index + 1? "bg-green-400": "bg-transparent"} flex justify-center hover:bg-blue-200 border-x border-gray-200 p-2 py-1 border-b-2 border-b-blue-600`} onClick={() => setChoice(choice === 0 ? -1 : index + 1)}>
                        <Image
                            alt="Question Image"
