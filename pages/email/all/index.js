...
function handleSubmit(e){
    ...
    .then((res) => router.push(`/email/all/${res.data.sender}`))
                ...
                <div className="container pt-0 mx-auto">
                    <div className="flex lg:w-5/6 w-full  flex-col mx-auto px-8 sm:space-x-4 sm:space-y-0 space-y-4 sm:px-0 items-end">
                        <div className="relative flex-grow w-full">
                            <label htmlFor="email"className="leading-7 text-sm text-gray-600"><span className='text-lg font-medium text-cyan-950'>Your Email</span> (use the email provided by organization)</label>
                        ...
                        <div className='flex flex-col py-2 w-full'>
                            <button onClick={handleSubmit} className="text-white w-full bg-green-500 border-0 py-2 px-8 focus:outline-none hover:bg-indigo-600 rounded text-lg font-bold">Send Verification Code</button>
                        </div>
                    </div>
                </div>
            </section>
